...
This script:
  ...
  2. Handles missing values (business-logic driven, not blind imputation)
  3. Cleans invalid values (e.g. negative charges)
  4. Engineers new features (tenure buckets, service counts, avg charge, etc.)
  5. Creates the two supervised business labels needed later:
  ...
  6. Saves the cleaned + engineered dataset to data/processed/
"""

import pandas as pd
import numpy as np

RAW_PATH = "/home/claude/CustomerRetentionAI/data/raw/customer_data_original.csv"
OUT_PATH = "/home/claude/CustomerRetentionAI/data/processed/customer_data_clean.csv"
...
def clean_data(df):
    ...
    # --- Fix negative / invalid Monthly_Charge (data entry errors) ---
    df.loc[df["Monthly_Charge"] < 0, "Monthly_Charge"] = np.nan
    df["Monthly_Charge"] = df["Monthly_Charge"].fillna(df["Monthly_Charge"].median())

    # --- Missing Value_Deal -> "No Deal" (customer not on a promo deal) ---
    df["Value_Deal"] = df["Value_Deal"].fillna("No Deal")

    # --- Missing Multiple_Lines -> "No" (no phone service means no multi-line) ---
    df["Multiple_Lines"] = df["Multiple_Lines"].fillna("No")

    # --- Internet-dependent add-ons: missing == customer has no internet service ---
    ...
    # --- Total_Charges / Total_Revenue sanity: clip negatives to 0 ---
    for col in ["Total_Charges", "Total_Refunds", "Total_Extra_Data_Charges",
                "Total_Long_Distance_Charges", "Total_Revenue"]:
        df[col] = df[col].clip(lower=0)

    # --- Drop exact duplicate customer rows if any ---
    before = len(df)
...
def engineer_features(df):
    ...
    # Binary churn label: Churned=1, everyone else (Stayed/Joined)=0
    ...
    # Count of subscribed add-on services (engagement/stickiness proxy)
    ...
    # Average revenue per month of tenure (avoids div by zero)
    df["Avg_Monthly_Revenue"] = df["Total_Revenue"] / df["Tenure_in_Months"].replace(0, 1)

    # Charge-to-revenue ratio (extra costs burden)
    ...
    # Refund ratio (dissatisfaction proxy)
    df["Refund_Ratio"] = df["Total_Refunds"] / df["Total_Revenue"].replace(0, 1)

    # Is on a long-term contract (stickiness)
    df["Is_Long_Term_Contract"] = (df["Contract"] != "Month-to-Month").astype(int)

    # Has a promotional deal
    df["Has_Value_Deal"] = (df["Value_Deal"] != "No Deal").astype(int)

    # Is high spender (top 25% monthly charge)
    q75 = df["Monthly_Charge"].quantile(0.75)
    df["Is_High_Spender"] = (df["Monthly_Charge"] >= q75).astype(int)

    # Referral engagement flag
    df["Has_Referrals"] = (df["Number_of_Referrals"] > 0).astype(int)

    # Senior citizen flag (business relevant for telecom)
    df["Is_Senior"] = (df["Age"] >= 60).astype(int)

    # ---------------------------------------------------------------
    # Customer_Category label (business-rule derived multiclass target
    # used later to TRAIN classification models, same approach the
    # original data uses for Churn_Category)
    # ---------------------------------------------------------------
    def categorize(row):
...
def main():
    ...
    import os
    os.makedirs("/home/claude/CustomerRetentionAI/data/processed", exist_ok=True)
